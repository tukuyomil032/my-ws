...
**Codex・Claude両対応のスキル運用。** 同じ知識収集基盤をCodexとClaudeの双方から実行できるようにし、モデル指定、プロジェクトルール、Git履歴まで再現可能な形で管理することに強い実装意欲を示している。

**ルーティン成果物のGit運用。** 自動更新されたファイルを内容別・タスク別に分けてコミットし、必要に応じてpushまで完了する運用を重視している。ナレッジベースを日次で育てるだけでなく、履歴の追跡性と再実行可能性を同じくらい重視している。
