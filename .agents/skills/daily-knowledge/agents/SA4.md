...
- Bloomberg 記事はペイウォールがある場合がある → WebSearch の snippet は候補発見にだけ使い、金額・評価額・投資家・見通しなどの主張は公開本文・一次発表・公式統計で確認できた範囲だけ要約する
...
- Jina Reader が失敗した場合、金額・評価額・投資家・経済指標などの数値は、公開本文・一次発表・公式統計で確認できるものだけ採用する。検索スニペットだけの強い数値主張は候補止まりにする。
