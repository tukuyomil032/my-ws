...
- 各SAを起動するとき、親は `OUTPUT_PATH=/tmp/daily-knowledge/YYYY-MM-DD/SA{N}.md` と `DONE_PATH=/tmp/daily-knowledge/YYYY-MM-DD/SA{N}.md.done` を明示して渡す。SAは `DONE_PATH` 以外の完了マーカーを作らず、別名マーカーだけが存在する成果物は未完了扱いにしてログへ記録する。
